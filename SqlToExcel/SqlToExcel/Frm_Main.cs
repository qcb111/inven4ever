...
namespace SqlToExcel
{
    public partial class Frm_Main : Form
    {
        ...
        #region 定义全局变量及对象
        string M_str_Con = "Data Source=DESKTOP-HMVGLVP;Database=Well_Info;Uid=sa;Pwd=94914699gY*;";//定义数据库连接字符串
        ...
        private DataSet SelectEInfo(string P_str_Condition, string P_str_KeyWord)
        {
            ...
            sqlda = new SqlDataAdapter("Select * from Well_Info", sqlcon);//实例化数据桥接器对象
            ...
            {
                case "井编号":
                    sqlcmd.Parameters.Add("@ID", SqlDbType.Int).Value = P_str_KeyWord;//为存储过程添加ID参数
                    sqlcmd.Parameters.Add("@Well_Number", SqlDbType.VarChar, 30).Value = "";//为存储过程添加Name参数
                    sqlcmd.Parameters.Add("@X_Coord", SqlDbType.VarChar,20).Value = "";//为存储过程添加坐标参数
                    sqlcmd.Parameters.Add("@Y_Coord", SqlDbType.VarChar,20).Value = "";
                    sqlcmd.Parameters.Add("@Well_Type", SqlDbType.VarChar, 20).Value = "";
                    sqlcmd.Parameters.Add("@Kelly_Bushing", SqlDbType.Float).Value = "";
                    sqlcmd.Parameters.Add("@Well_Type", SqlDbType.Float).Value = "";
                    sqlcmd.Parameters.Add("@Finish_Date", SqlDbType.DateTime).Value = "";
                    break;
                case "井名":
                    sqlcmd.Parameters.Add("@ID", SqlDbType.Int).Value = "";//为存储过程添加ID参数
                    sqlcmd.Parameters.Add("@Well_Number", SqlDbType.VarChar, 30).Value = P_str_KeyWord;//为存储过程添加Name参数
                    sqlcmd.Parameters.Add("@X_Coord", SqlDbType.VarChar,20).Value = "";//为存储过程添加坐标参数
                    sqlcmd.Parameters.Add("@Y_Coord", SqlDbType.VarChar,20).Value = "";
                    sqlcmd.Parameters.Add("@Well_Type", SqlDbType.VarChar, 20).Value = "";
                    sqlcmd.Parameters.Add("@Kelly_Bushing", SqlDbType.Float).Value = "";
                ...
                case "井型":
                    ...
                    sqlcmd.Parameters.Add("@X_Coord", SqlDbType.VarChar,20).Value = "";//为存储过程添加坐标参数
                    sqlcmd.Parameters.Add("@Y_Coord", SqlDbType.VarChar,20).Value = "";
                    sqlcmd.Parameters.Add("@Well_Type", SqlDbType.VarChar, 20).Value = P_str_KeyWord;
                    sqlcmd.Parameters.Add("@Kelly_Bushing", SqlDbType.Float).Value = "";
                    sqlcmd.Parameters.Add("@Well_Type", SqlDbType.Float).Value = "";
                    sqlcmd.Parameters.Add("@Finish_Date", SqlDbType.DateTime).Value = "";
                    break;
